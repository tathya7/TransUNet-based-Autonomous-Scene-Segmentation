...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.skip_channels = skip_channels  # Store for reference
    ...
    def forward(self, x, skip=None):
        ...
        if skip is not None:
            ...
            # If skip is None, pad x with zeros to match expected channels
            if self.skip_channels > 0:
                B, C, H, W = x.size()
                padding = torch.zeros(
                    B, self.skip_channels, H, W, device=x.device, dtype=x.dtype
                )
